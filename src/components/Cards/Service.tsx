...
const Service = ({ title, description, image, link, color }: { title: string; description: string; image: string; link: string; color: string }) => {
    return (
        <Card className={`border border-divider bg-gradient-to-t ${color} p-4 shadow-none drop-shadow-sm transition-all duration-300 hover:-translate-y-4 hover:drop-shadow-xl`}>
