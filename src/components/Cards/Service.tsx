...
const Service = ({ title, description, image, link, color }: { title: string; description: string; image: string; link: string; color: string }) => {
    return (
        <Card className={`border border-divider bg-gradient-to-t ${color} transform-gpu p-4 shadow-sm transition-all duration-300 hover:-translate-y-4 hover:shadow-xl`}>
